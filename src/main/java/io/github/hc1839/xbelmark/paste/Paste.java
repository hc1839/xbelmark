...
public class Paste {
   ...
   */
  public static final Pattern RESERVED_RE =
      Pattern.compile("<|>|:|\"|/|\\\\|\\||\\?|\\*");
  ...
  static {
    ...
    Option formatOption = Option
        ...
        .longOpt("format")
        .desc(
            "Format of the output file. Valid values are `URL` (URL format "
            + "used in Windows systems) and `XBEL` (XBEL format). If not "
  ...
  public static void Execute(String[] args) {
    ...
    if (cmdline.hasOption("f")) {
      bookmarkFormat = Format.valueOf(cmdline.getOptionValue("format"));
    } else {
      bookmarkFormat = DEFAULT_FORMAT;
    }

    String bookmarkUriString = cmdline.getOptionValue("uri");

    // Get the URL from the clipboard if one was not specified on the command
    // line.
    if (bookmarkUriString == null) {
      try {
        Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
        bookmarkUriString =
            clipboard.getData(DataFlavor.stringFlavor).toString().strip();
    ...
    if (isStdout) {
      ...
      bookmarkFileName = bookmarkFileName.replaceAll("\\s+", " ");
      ...
      Path bookmarkFilePath =
          Paths.get("").toAbsolutePath().resolve(bookmarkFileName);
