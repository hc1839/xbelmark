...
public enum Format {
  ...
  XBEL {
    ...
    public String text(URI uri, String htmlTitle) {
      Document xbel;
      try {
        xbel = DocumentBuilderFactory.newInstance().newDocumentBuilder().newDocument();
      } catch (Exception e) {
        throw new RuntimeException("Cannot create the builder for the XBEL document.");
      }
      ...
      bookmark.setAttribute(
          "added", DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(ZonedDateTime.now().withNano(0)));
      ...
      try {
        ...
        transformer.transform(new DOMSource(xbel), new StreamResult(stringWriter));
      } catch (Exception e) {
